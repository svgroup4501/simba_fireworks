...
<body>
    ...
    @if (!empty($company_collection) && $company_collection->count())
        @foreach ($company_collection as $array_collection)
            <p style="text-align:left;">Company Name: {{ $array_collection->{COMPANY_NAME} }}</p>
            <p style="text-align:left;">Company Address:
                {{ $array_collection->{COMPANY_ADDRESS} ? $array_collection->{COMPANY_ADDRESS} : '------' }}</p>
            <p style="text-align:left;">Company GST:
                {{ $array_collection->{COMPANY_GST} ? $array_collection->{COMPANY_GST} : '------' }}</p>
        @endforeach
    @endif
            ...
            @if (!empty($account_collection) && $account_collection->count())
                ...
                @foreach ($account_collection as $array_collection)
                    <tr>
                        ...
                        <td style="width:20%;margin-left:20px">
                            {{ $array_collection->{CUSTOMER_NAME} }}</td>
                        @php
